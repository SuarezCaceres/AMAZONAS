...
## Corpus Check
- 245 files · ~122,942 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 602 nodes · 1060 edges · 42 communities (27 shown, 15 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 24 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f60ad69e`
...
## Knowledge Gaps
- **23 isolated node(s):** `Regla Graphify (lectura obligatoria antes de buscar)`, `1. Archivos en la Raíz del Proyecto`, `2. Archivos de Salida y Reportes (Carpeta `src/graphify-out/`)`, `3. Integración Continua (Hooks de Git)`, `Requerimientos Clave:` (+18 more)
...
- **Why does `SolicitudesComponent` connect `SolicitudesComponent` to `ChatServiceImpl`, `.abrirModalCancelacion`, `.addPreloadedFile`, `.clearPreloadedFiles`, `ChatMessageResponse`, `.loadSolicitudes`, `.onConfirmarPagoDesdeChat`?**
  _High betweenness centrality (0.160) - this node is a cross-community bridge._
- **What connects `Regla Graphify (lectura obligatoria antes de buscar)`, `1. Archivos en la Raíz del Proyecto`, `2. Archivos de Salida y Reportes (Carpeta `src/graphify-out/`)` to the rest of the system?**
